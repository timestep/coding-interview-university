...
- ### Arrays
  ...
  - [ ] Space
    - contiguous in memory, so proximity helps
        1 >= n) * size of item, but even if 2n, still O(n)

- ### Linked Listsn

  Its OBJ -> OBJ Refernces
  - [x] Description:
...
- ### Hash table
  ...
  - [ ] Videos:
    - [x] [Hashing with Chaining (video)](https://www.youtube.com/watch?v=0M_kIqhwbFo&list=PLUl4u3cNGP61Oq3tWYp6V_F-5jb5L2iHb&index=8)
      - NOTE: The algorithm to find h. Is that nessecary???? Very Theoretical.
  ...
  - [x] Implement with array using linear probing
    ...
    - get(key)
    - remove(key)
  - [x] Solve Rolling Hash to find s: string in t: string; (Karp-Rabin)
...
- ### Bitwise operations (WORK ON THIS)
